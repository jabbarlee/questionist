...
export const getAllResults = async (uid: string) => {
  try {
    const practiceSessionsRef = collection(
      doc(db, "users", uid),
      "practiceSessions"
    );
    ...
    const practiceSessions: { id: string }[] = querySnapshot.docs.map(
      (doc) => ({
...
export const fetchResults = async ({
  ...
  try {
    const allSessions = await getAllResults(uid);

    // Transform the data to match the updated `SessionData` type
    const transformedSessions = allSessions?.map(
