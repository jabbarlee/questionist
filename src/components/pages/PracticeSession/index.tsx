...
export default function Index({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    getSessionData({ uid, sessionId, setSessionData });
  }, [sessionId]);
  ...
  const handleSubmit = async () => {
    const selectedChoices: SelectedOption[] = questions.map((question) => {
      ...
      const selectedOptionText =
        question.choices.find((opt) => opt.id === selectedOptionId)?.text || "";
    ...
    if (success) {
      ...
      setMessage(
        "Session submitted successfully! Redirecting to results page..."
      );
        ...
        <div className={styles.questionsWrapper}>
          {loading ? (
            ...
            <Question
              ...
              selectedOption={
                selectedOptions[questions[currentPage - 1]?.id] || null
              }
